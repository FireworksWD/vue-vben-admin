...
import type { DictionaryStates } from './interface';
...
export const DictionaryStore = defineStore('Dictionary', {
	...
	actions: {
      ...
      requestClient.get('/api/init/dictionary/?dictionary_key=all').then((ret: { data: [] }) => {
				// 转换数据格式并保存到pinia
				...
				dataList.forEach((item: any) => {
					...
					childrens.forEach((children:any, _index:any) => {
