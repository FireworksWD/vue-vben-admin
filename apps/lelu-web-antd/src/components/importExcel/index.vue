      ...
      >
        <div
          ...
          <div style="display: flex;gap: 10px">
            <a-button
              ...
              class="gradient-hover">{{ $t('system.N00010') }}</a-button>
            <a-button
              type="primary"
              @click="updateTemplate"
              class="gradient-hover">{{ $t('system.N00011') }}</a-button>
          </div>
          ...
          <div style="display: flex;gap: 10px">
            ...
            <a-button type="primary" :disabled="fileList?.length === 0" @click="submitUpload">{{ $t('system.N00181') }}</a-button>
        ...
        >
              ...
              <a-space align="start"
                ...
                <div style="display: flex; align-items: center;">
                  <a-tooltip :title="$t('system.N00375')">
                    <a-button danger v-if="file.customStatus==='pending'" @click="removeFile(file, actions.remove)">
          ...
          <div
            ...
            <!-- 图标居中 -->
            <p>
              <Icon icon="line-md:uploading-loop" style="font-size: 50px;"/>
            </p>
            ...
            <div style="color:red">提示：仅允许导入“xls”或“xlsx”格式文件！最多支持100个文件同时上传
            </div>
...
const acceptType = ref(['.xls', '.xlsx', '.csv'])
...
const importTemplate = function () {
  downloadFile(props.api + 'import_data/', {
    params: {},
    method: 'get'
  })
}
...
const updateTemplate = function () {
  downloadFile(props.api + 'update_template/', {
    params: {},
    method: 'get',
    filename: $t('system.N00212')
  })
}
/**
 * 上传前的处理
...
const handleUploadChange = async (info: UploadChangeParam) => {
  if (info.file.status === 'done') {
    loading.value = true
    await requestClient.post(props.api + 'import_data/', {url: info.file.response.data.url})
    loading.value = false
  }
}
/**
...
const submitUpload = async () => {
  try {
    ...
    loading.value = true
    await Promise.all(
      successFile.map(file =>
        requestClient.post(props.api + 'import_data/', { url: file.response.data.file_url })
      )
    );
    loading.value = false
    message.info(`${uploadPromises.length ?? 0} 个文件全部处理完成`);
  }catch (error) {
