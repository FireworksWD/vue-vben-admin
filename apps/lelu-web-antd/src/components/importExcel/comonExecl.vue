<template>
  <div style="display: inline-block">
    <a-button type="primary" @click="handleImport()" :icon="h(CloudUploadOutlined)">
        ...
        <div
          ...
          <div style="display: flex;gap: 10px">
            <a-button
              ...
              class="gradient-hover">{{ $t('system.N00010') }}
            </a-button>
          </div>
          ...
          <div style="display: flex;gap: 10px">
            ...
            <a-button @click="uploadShow=false">{{ $t('system.N00091') }}</a-button>
          </div>
        </div>
        <a-upload-dragger
          multiple
  ...
  try {
    ...
    await Promise.all(
      successFile.map(file =>
        requestClient.post(props.api, {url: file.response.data.file_url})
      )
    );
    loading.value = false
